...
namespace AZ.TcpNet.Base
{
    ...
    /// 
    /// </summary>
    public class AzTcpClient : IDisposable
    {
        /// <summary>
        /// 
        /// </summary>
        ...
        private readonly TcpClient _workClient;
        /// <summary>
        /// 连接成功标识
        /// </summary>
        ...
        public bool KeepAlive
        {
            ...
            set
            {
                if (!_keepAlive && value)
                {
                    _workClient.Client.SetKeepAlive();
                }
        ...
        /// 接收到数据事件
        /// </summary>
        public event Action<AzReceiveEventArgs> ReceivedData;
        ...
        public AzTcpClient(int bodyHeadLength=4,int cmdCodeHeadLength=4,int receiveBufferSize=8192)
        {
            Id = Guid.NewGuid();

            _workClient = new TcpClient();
        ...
        /// <param name="cmdCodeHeadLength">消息编号在header中的字节数</param>
        /// <param name="receiveBufferSize">消息接收缓冲buffer长度</param>
        public AzTcpClient(TcpClient tcpClient, int bodyHeadLength = 4, int cmdCodeHeadLength = 4, int receiveBufferSize = 8192)
        {
            _workClient = tcpClient;
            _workClient.NoDelay = true;
            Id = Guid.NewGuid();

            _isConnectionSuccessful = true;
            _ipEndPoint = tcpClient.GetRemoteIpEndPoint();
        ...
        public AzTcpClient(IPEndPoint ipEndPoint, int bodyHeadLength = 4, int cmdCodeHeadLength = 4, int receiveBufferSize = 8192, int timeoutMSec = 1000)
        {
            Id = Guid.NewGuid();
            _workClient = new TcpClient();
        ...
        protected void OnReceivedData(int cmdType,byte[] data)
        {
            ...
            var args = new AzReceiveEventArgs()
            {
        ...
        /// 
        /// </summary>
        public event EventHandler<AzConnectEventArgs> ClientDisconnected;
        /// <summary>
        /// 
        /// </summary>
        /// <param name="e"></param>
        protected virtual void OnClientDisconnected(AzConnectEventArgs e)
        {
        ...
        /// 
        /// </summary>
        public event EventHandler<AzConnectEventArgs> ClientConnected;
        /// <summary>
        /// 
        /// </summary>
        protected virtual void OnClientConnected(AzConnectEventArgs e)
        {
            try
            {
                KeepAlive = true;
        ...
        /// 
        /// </summary>
        public event EventHandler<AzConnectEventArgs> ConnectTimeout;
        /// <summary>
        /// 
        /// </summary>
        protected virtual void OnConnectTimeout(AzConnectEventArgs e)
        {
        ...
        /// 
        /// </summary>
        public event EventHandler<AzConnectEventArgs> OccurredSocketErrors;
        /// <summary>
        /// 
        /// </summary>
        protected virtual void OnOccurredSocketErrors(AzConnectEventArgs e)
        {
        ...
        public void Connect(IPEndPoint remoteEndPoint, int timeoutMSec = 1000)
        {
            ...
            if (timeoutMSec == 0)
            {
                try
                {
                    ...
                    //连接成功,触发已连接事件
                    OnClientConnected(new AzConnectEventArgs() { ClientId = Id, RemoteEndPoint = GetRemoteIpEndPoint(), Status = AzSocketStateEnum.Normal });
                }
                catch (Exception ex)
                {
                    OnClientDisconnected(new AzConnectEventArgs() { ClientId = Id, RemoteEndPoint = GetRemoteIpEndPoint(), Status = AzSocketStateEnum.SocketError, Exception = ex });
                }
            }
            else
            {
                ...
                else
                {
                    ...
                    OnClientDisconnected(new AzConnectEventArgs() { ClientId = Id, RemoteEndPoint = GetRemoteIpEndPoint(), Status = AzSocketStateEnum.ConnectTimeout, Exception = null });
                }
            }
        }
        ...
        private void CallBackMethod(IAsyncResult asyncresult)
        {
            try
            {
                ...
                var tcpclient = asyncresult.AsyncState as AzTcpClient;
        ...
        public bool IsConnected()
        {
            ...
            if (!ret)
            {
                OnClientDisconnected(new AzConnectEventArgs() { ClientId = Id, RemoteEndPoint = GetRemoteIpEndPoint(), Status = AzSocketStateEnum.Normal });
            }
            return ret;
        }
        ...
        public void BeginReceiveData()
        {
            try
            {
                ...
                _receiveCallback = _workClient.Client.BeginReceive(_buffer, 0, BUFFER_SIZE, SocketFlags.None, ReceivedCallback, _workClient);
        ...
        private void ReceivedCallback(IAsyncResult ar)
        {
            ...
            try
            {
                ...
                if (client != null)
                {
                    ...
                    {
                        //连接断开
                        OnClientDisconnected(new AzConnectEventArgs() { ClientId = Id, RemoteEndPoint = GetRemoteIpEndPoint(), Status = AzSocketStateEnum.RemoteClose });
                        return;
                    }

                    receivedCount = client.Client.EndReceive(ar);
                }
                else
                {
                    OnClientDisconnected(new AzConnectEventArgs() { ClientId = Id, RemoteEndPoint = GetRemoteIpEndPoint(), Status = AzSocketStateEnum.RemoteClose, Exception = new IOException("client is null") });
                }
            ...
            catch (Exception ex)
            {
                ...
                {
                    OnClientDisconnected(new AzConnectEventArgs()
                    {
                        ClientId = Id,
        ...
        public void SendAsync(byte[] buffer)
        {
            try
            {
                ...
                _workClient.Client.BeginSend(buffer, 0, buffer.Length, SocketFlags.None, asyncCallback, this);
            }
            catch (Exception ex)
            {
                OnClientDisconnected(new AzConnectEventArgs() { ClientId = Id, RemoteEndPoint = GetRemoteIpEndPoint(), Status = AzSocketStateEnum.SocketError, Exception = ex });
        ...
        public byte[] Send(byte[] buffer)
        {
            try
            {
                if (_workClient != null && _workClient.IsConnected())
                {
                    var outputStream = _workClient.GetStream();

                    ////发送数据
                    outputStream.Write(buffer, 0, buffer.Length);

                    ////开始接收数据,若服务器暂时未返回,则会等待
                    var inPutStream = _workClient.GetStream();
